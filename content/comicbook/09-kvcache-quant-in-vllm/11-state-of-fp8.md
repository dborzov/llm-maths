...
This capstone has three parts. First, the full benchmark table — what FP8 KV-cache does on each model the team validated. Second, the three exceptions — when not to enable it. Third, the connection back to [Issue 3's KV method family tree](/comicbook/03-quantization/15-kv-method-family/) and the broader story of where FP8 fits among the eight modern KV quantization methods.
...
Back to [Issue 3's KV method family tree](/comicbook/03-quantization/15-kv-method-family/). That tree, in May 2026, lists eight methods: FP8, KIVI (2-bit), KVQuant (4-bit), GEAR (low-rank + sparse + 4-bit), ATOM (W4A4KV4), QServe (W4A8KV4), KVTuner (per-layer adaptive bits), TurboQuant (random rotations). The selection table at the end of that article asks: which one do you actually use?
...
This issue is, in a structural sense, the same story as [Issue 6's KVzap journey](/comicbook/06-kvcache-pruning/) and [Issue 7's DeepSeek-V4 arc](/comicbook/07-deepseek-attn/). Each one is about an inference optimization that *appeared* to work for years and was eventually discovered to be incomplete in some specific way, and each one is about the careful engineering work required to make the optimization actually-work, not just plausibly-work.
...
- [Issue 3 — Sixteen Numbers Walk Into A GPU](/comicbook/03-quantization/) for the full quantization landscape, the [KV method family tree](/comicbook/03-quantization/15-kv-method-family/), the [number-format primer](/comicbook/03-quantization/02-numbers-in-boxes/), and the [hardware horizon](/comicbook/03-quantization/12-hardware-horizon/) that places Hopper and Blackwell in context.
- [Issue 6 — The Eviction Notice](/comicbook/06-kvcache-pruning/) for the complementary KV-pruning story and the [bandwidth wall primer](/comicbook/06-kvcache-pruning/11-bandwidth-wall/).
- [Issue 7 — The Sparse Lab](/comicbook/07-deepseek-attn/) for DeepSeek-V4's mixed-precision KV policy and the broader picture of long-context inference economics.
- [Issue 8 — Anatomy of a Token](/comicbook/08-vLLM/) for the [FlashAttention primer](/comicbook/08-vLLM/06-flash-attention/), the [roofline model](/comicbook/08-vLLM/04-roofline/), and the [prefill / decode split](/comicbook/08-vLLM/07-prefill-vs-decode/) that frame the bandwidth-bound regime FP8 KV-cache lives in.
