...
- Why **Blackwell (B200)** does not need the two-level accumulation fix — what changed in the silicon, and what that says about the silicon-software co-evolution we've been tracing since [Issue 3's Hardware Horizon](/comicbook/03-quantization/12-hardware-horizon/).
- How FP8 KV fits into the broader [KV method family tree](/comicbook/03-quantization/15-kv-method-family/) from Issue 3 — and why the simplest method on the shelf became the one that took two years and a numerical-analysis paper to finish properly.
