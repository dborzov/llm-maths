...
Calibration as a concept is old. It is one of the central topics in [Issue 3's calibration field guide](/comicbook/03-quantization/13-calibration-survey/), which traces five families of calibration approaches:

1. **Data-free** — analytic methods that compute scales without any input data (Lloyd-Max for uniform quantization, the symmetric scaling of [absmax](/comicbook/03-quantization/02a-absmax/)).
...
This is the architectural reason FP8 KV is the "easy" entry on [the KV method family tree](/comicbook/03-quantization/15-kv-method-family/) — the format gives you most of the slack for free.
{{% /callout %}}
...
**Continue to** → [The State of FP8 KV](../11-state-of-fp8/) — the boss capstone. The full report card across models and benchmarks, the three places FP8 KV still doesn't fit, and the connection back to [Issue 3's KV method family tree](/comicbook/03-quantization/15-kv-method-family/).
