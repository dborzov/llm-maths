...
This is the same pattern FA-3 itself introduced (see [Issue 8's FlashAttention primer](/comicbook/08-vLLM/06-flash-attention/)) — warp specialization plus async TMA — applied specifically to the decode tile pattern. The decode slope improvement compounds with PR #91's contribution.
...
To make the tile-size cost concrete, here is a Roofline-style picture of the FA3 FP8 kernel's effective throughput as a function of register pressure. [Issue 8's roofline primer](/comicbook/08-vLLM/04-roofline/) introduces the model; this is its application.
