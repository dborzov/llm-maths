...
Modern LLM inference runs in two phases: **prefill** (process the input prompt, populate the {{< wiki "kv-cache" >}}KV cache{{< /wiki >}}) and **decode** (generate tokens one at a time, extending the cache with each step). [Issue 8's primer](/comicbook/08-vLLM/07-prefill-vs-decode/) walks through both at length, so here is the one-paragraph summary needed for the bargain.
...
This is the **arithmetic intensity** picture that drove FlashAttention in the first place. [Issue 8's roofline primer](/comicbook/08-vLLM/04-roofline/) and [Issue 6's bandwidth wall](/comicbook/06-kvcache-pruning/11-bandwidth-wall/) both unpack the math: decode attention sits at roughly $I \approx 1$ FLOP per byte, while the H100 ridge point is roughly $\sim 200$ FLOPs per byte. Decode is bandwidth-bound by a factor of **~200×**. Most of the tensor core silicon is sitting idle during decode, waiting for the cache to land in SRAM.
