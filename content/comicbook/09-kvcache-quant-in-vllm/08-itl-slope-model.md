...
**3. KV-cache spilling.** At very long contexts, the cache may exceed GPU memory and the system has to fall back to either CPU RAM, paged KV (which has its own latency profile, see [Issue 8 ch.10](/comicbook/08-vLLM/10-paged-attention/)), or eviction (see [Issue 6](/comicbook/06-kvcache-pruning/)). All three add per-step costs that aren't linear in T, so the model breaks at the spill point.
