...
6. **Every weight matrix in the model is the same operation in a different costume.** `attn_wq`, `mlp_fc1`, and `lm_head` are mechanically identical; only the contents of `w` differ. This is why [quantizing one weight matrix](../../03-quantization/) is the same problem as quantizing all of them.
