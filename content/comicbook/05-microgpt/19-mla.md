...
MLA solved the *memory* problem. It did not solve the *compute* problem — attention is still $O(T^2)$ per layer regardless of how small the cache is. DeepSeek's subsequent papers (NSA, DSA in V3.2-Exp, then CSA + HCA in V4) attack the compute side. The trajectory is the subject of **[Issue 7: The Sparse Lab](/comicbook/07-deepseek-attn/)**:

- The same K/V latent that MLA introduced here becomes the *query latent* $c^Q_t$ that drives the **lightning indexer** in V3.2-Exp's DSA — [Issue 7 ch.6](/comicbook/07-deepseek-attn/06-lightning-indexer/).
- The RoPE-vs-NOPE split from this chapter becomes the architectural seed for the indexer's separate scoring head — [Issue 7 ch.2 (MLA, rewound)](/comicbook/07-deepseek-attn/02-mla-rewind/).
- V4 wraps the whole machinery in a **token-level compressor** (CSA) — [Issue 7 ch.7](/comicbook/07-deepseek-attn/07-csa/).
{{% /callout %}}
