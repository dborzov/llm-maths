...
The attention weight $\alpha_{ij}$ is the dot product of token $i$'s **query** vector (`q` in the [microGPT reference](../../05-microgpt/08-attention/)) and token $j$'s **key** vector (`k`), normalised by {{< wiki "softmax" >}}softmax{{< /wiki >}} — these are the `attn_logits` and `attn_weights` from the listing:
