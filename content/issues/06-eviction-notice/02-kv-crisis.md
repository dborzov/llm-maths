...
This is not a research problem. It is a product-shipping problem, and it is hitting ML engineers in March 2023 at companies ranging from Google to startups with two-person inference teams. The KV cache — once a routine implementation detail in the [ch.13 KV Cache](/issues/05-microgpt-unfolded/13-kv-cache/) of every transformer textbook — has become the dominant cost at scale.
...
The original **multi-head attention** design from [Inside K and V](/issues/03-sixteen-numbers/16-kv-distribution/) uses a separate key and value head for every query head. If you have 64 query heads, you have 64 KV heads. Each head independently learns its own projection and stores its own cache.
