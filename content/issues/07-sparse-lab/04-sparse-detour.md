...
In [Issue 6's KVzap chapter](/issues/06-eviction-notice/06-kvzap/), we established four criteria that any production inference component must pass. Every {{< wiki "attention" >}}attention{{< /wiki >}} mechanism in a frontier model gets tested against this list whether the paper authors knew it or not:
...
4. **The eviction/sparsity distinction matters.** H₂O and SnapKV are eviction methods and belong to [Issue 6](/issues/06-eviction-notice/06-kvzap/). They fail on different criteria than sparse attention methods.
