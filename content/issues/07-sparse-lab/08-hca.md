...
The MLA absorption trick still applies: $W^{UK}$ can be absorbed into the query side, so the compressed KV cache stores the raw compressed latents and the projection is fused at decode time. The [Issue 5 ch.19 MLA](/issues/05-microgpt-unfolded/19-mla/) article covers this absorption in detail; the same algebra applies to the compressed entries here.
