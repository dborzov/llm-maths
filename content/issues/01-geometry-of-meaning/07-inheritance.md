...
- [Issue 2: The Logit Wager](../../02-logit-wager/) — what the transformer does *after* the embedding lookup
- [Issue 5: microGPT Unfolded](../../05-microgpt-unfolded/) — the full transformer forward pass, with `wte` and `wpe` named and traced

</details>

**Continue to →** [Issue 2: The Logit Wager](../../02-logit-wager/), where we follow the embedded vectors through the rest of the transformer — and discover that the next operation (the linear classifier head) is just as geometrically elegant as the embedding that feeds it.
