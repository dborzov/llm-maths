...
In [microGPT's listings](../../05-microgpt-unfolded/16-full-forward/), the `keys[li].append(k)` and `values[li].append(v)` lines are where these shapes are born. This asymmetry is the load-bearing fact for [per-channel KV quantization](../../03-sixteen-numbers/18-quantization-axes/).
...
The reason is mathematical. In the [microGPT attention loop](../../05-microgpt-unfolded/08-attention/), the `softmax(attn_logits)` function forces the weights to sum exactly to **1.0**. 
